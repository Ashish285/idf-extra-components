...
![RSA Image Format](https://raw.githubusercontent.com/espressif/idf-extra-components/master/esp_encrypted_img/image_format.png)

The header for an image encrypted using the RSA-3072 scheme is structured as follows:

```c
typedef struct {
    uint8_t magic[4];             // Magic bytes (e.g., derived from SHA256("esp_encrypted_img_rsa"))
...
![ECIES-P256 Image Format](ecc_image_format.png)
...
* **Reserved Key Parameters (288 bytes):** Padding to align the key parameters section (server public key, KDF salt, and this field) to 384 bytes.
* **IV (16 bytes):** The Initialization Vector used for AES-GCM encryption.
* **Binary Size (4 bytes):** The size of the original, unencrypted binary data, in little-endian format.
* **Auth Tag (16 bytes):** The authentication tag generated by AES-GCM.
* **Reserved Final Padding (88 bytes):** Additional padding to ensure the total header size is 512 bytes.
