...
#if defined(CONFIG_PRE_ENCRYPTED_RSA_USE_DS)
    int rsa_padding; /*!< Padding type for RSA decryption */
#else
    char *rsa_pem;
    size_t rsa_len;
#endif
...
static int decipher_gcm_key(const char *enc_gcm, esp_encrypted_img_t *handle)
{
    ...
    mbedtls_pk_init(&pk);
    int padding = handle->rsa_padding;
    if (padding < 0 || padding > MBEDTLS_RSA_PKCS_V21) {
        ESP_LOGE(TAG, "Invalid padding type: %d", padding);
        return ESP_ERR_INVALID_ARG;
    }
    ret = mbedtls_pk_setup_rsa_alt(&pk, &padding, esp_ds_rsa_decrypt, NULL, esp_ds_get_keylen);
    if (ret != 0) {
...
esp_decrypt_handle_t esp_encrypted_img_decrypt_start(const esp_decrypt_cfg_t *cfg)
{
    ...
    handle->rsa_padding = cfg->rsa_padding;
    if (handle->rsa_padding < 0 || handle->rsa_padding > MBEDTLS_RSA_PKCS_V21) {
        ESP_LOGE(TAG, "esp_encrypted_img_decrypt_start : Invalid padding type");
        goto failure;
    }
#else
